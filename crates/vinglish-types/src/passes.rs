            ...
            match item {
                ...
                AstItem::Use(u) => {
                    let path_parts: Vec<String> =
                        u.path.iter().map(|id| id.name.to_string()).collect();
                ...
                AstItem::ForeignImport(f) if f.lang.name.as_ref() == "c" => {
                    // Extract functions using bindgen and syn
                    let bindings_str = match bindgen::builder().header(&f.path).generate() {
                        Ok(b) => b.to_string(),
                        Err(_) => {
    ...
    match ty {
        syn::Type::Path(type_path) => {
            if let Some(segment) = type_path.path.segments.last() {
                ...
                match ident.as_str() {
                    "i32" | "i64" | "u32" | "u64" | "isize" | "usize" | "c_int" | "c_long"
                    | "c_longlong" => Type::Int,
