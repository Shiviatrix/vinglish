...
impl Healer {
    ...
    pub fn candidates(&self, expr: &Expr, constraint: &TypeConstraint) -> Vec<HealingCandidate> {
        ...
        // AutoDeref: when we have &T but need T
        if let Type::Reference(inner, _) = &constraint.actual
            && **inner == constraint.expected
        {
        ...
        // ToText: when we need Text but have something else
        ...
        // AutoWiden: Int to Float/Decimal
...
fn find_expr(module: &Module, id: AstNodeId) -> Option<&Expr> {
    for item in &module.items {
        let block = match item {
            ...
            Item::Route(r) => Some(&r.handler),
            Item::Statement(s) => return find_expr_in_stmt(s, id),
            _ => None,
        };
        if let Some(block) = block
            && let Some(expr) = find_expr_in_block(block, id)
        {
            return Some(expr);
        }
    }
...
fn find_expr_mut(module: &mut Module, id: AstNodeId) -> Option<&mut Expr> {
    for item in &mut module.items {
        let block = match item {
            Item::Function(f) => Some(&mut f.body),
            Item::Route(r) => Some(&mut r.handler),
            Item::Statement(s) => return find_expr_in_stmt_mut(s, id),
            _ => None,
        };
        if let Some(block) = block
            && let Some(expr) = find_expr_in_block_mut(block, id)
        {
            return Some(expr);
        }
    }
...
fn find_expr_in_block_mut(block: &mut Block, id: AstNodeId) -> Option<&mut Expr> {
    for stmt in &mut block.stmts {
        if let Some(expr) = find_expr_in_stmt_mut(stmt, id) {
            return Some(expr);
        }
    }
    None
...
fn find_expr_in_stmt(stmt: &Stmt, id: AstNodeId) -> Option<&Expr> {
    match stmt {
        ...
        Stmt::If(s) => find_expr_in_expr(&s.condition, id)
            .or_else(|| find_expr_in_block(&s.then_block, id))
            .or_else(|| s.otherwise.as_ref().and_then(|b| find_expr_in_block(b, id))),
        Stmt::When(s) => find_expr_in_expr(&s.condition, id)
            .or_else(|| find_expr_in_block(&s.then_block, id))
            .or_else(|| s.otherwise.as_ref().and_then(|b| find_expr_in_block(b, id))),
        Stmt::Assign(s) => {
            find_expr_in_expr(&s.target, id).or_else(|| find_expr_in_expr(&s.value, id))
        }
        Stmt::Expr(e) | Stmt::Send(vinglish_parser::ast::SendStmt { message: e, .. }) => {
            find_expr_in_expr(e, id)
        }
        Stmt::Transaction(t) => find_expr_in_block(&t.body, id),
        Stmt::Repeat(r) | Stmt::ParallelRepeat(r) => match r {
            vinglish_parser::ast::RepeatStmt::ForEvery { iterable, body, .. } => {
                find_expr_in_expr(iterable, id).or_else(|| find_expr_in_block(body, id))
            }
            vinglish_parser::ast::RepeatStmt::While {
                condition, body, ..
            } => find_expr_in_expr(condition, id).or_else(|| find_expr_in_block(body, id)),
            vinglish_parser::ast::RepeatStmt::Count { times, body, .. } => {
                find_expr_in_expr(times, id).or_else(|| find_expr_in_block(body, id))
            }
        },
        Stmt::Match(m) => find_expr_in_expr(&m.subject, id)
            .or_else(|| m.cases.iter().find_map(|c| find_expr_in_block(&c.body, id)))
            .or_else(|| m.otherwise.as_ref().and_then(|b| find_expr_in_block(b, id))),
        _ => None,
    }
}
...
fn find_expr_in_stmt_mut(stmt: &mut Stmt, id: AstNodeId) -> Option<&mut Expr> {
    match stmt {
        ...
        Stmt::If(s) => {
            if let Some(e) = find_expr_in_expr_mut(&mut s.condition, id) {
                return Some(e);
            }
            if let Some(e) = find_expr_in_block_mut(&mut s.then_block, id) {
                return Some(e);
            }
            s.otherwise
                .as_mut()
                .and_then(|b| find_expr_in_block_mut(b, id))
        }
        Stmt::When(s) => {
            if let Some(e) = find_expr_in_expr_mut(&mut s.condition, id) {
                return Some(e);
        ...
        Stmt::Assign(s) => {
            if let Some(e) = find_expr_in_expr_mut(&mut s.target, id) {
                return Some(e);
            }
            find_expr_in_expr_mut(&mut s.value, id)
        }
        Stmt::Expr(e) | Stmt::Send(vinglish_parser::ast::SendStmt { message: e, .. }) => {
            find_expr_in_expr_mut(e, id)
        }
        Stmt::Transaction(t) => find_expr_in_block_mut(&mut t.body, id),
        Stmt::Repeat(r) | Stmt::ParallelRepeat(r) => match r {
            vinglish_parser::ast::RepeatStmt::ForEvery { iterable, body, .. } => {
                if let Some(e) = find_expr_in_expr_mut(iterable, id) {
                    return Some(e);
                }
                find_expr_in_block_mut(body, id)
            }
            vinglish_parser::ast::RepeatStmt::While {
                condition, body, ..
            } => {
                if let Some(e) = find_expr_in_expr_mut(condition, id) {
                    return Some(e);
                }
                find_expr_in_block_mut(body, id)
            }
            vinglish_parser::ast::RepeatStmt::Count { times, body, .. } => {
                if let Some(e) = find_expr_in_expr_mut(times, id) {
                    return Some(e);
                }
                find_expr_in_block_mut(body, id)
            }
        ...
        Stmt::Match(m) => {
            if let Some(e) = find_expr_in_expr_mut(&mut m.subject, id) {
                return Some(e);
            }
            for case in &mut m.cases {
                if let Some(e) = find_expr_in_block_mut(&mut case.body, id) {
                    return Some(e);
                }
            }
            m.otherwise
...
fn find_expr_in_expr(expr: &Expr, id: AstNodeId) -> Option<&Expr> {
    if matches_id(expr, id) {
        return Some(expr);
    }
    match expr {
        Expr::Call { callee, args, .. } => find_expr_in_expr(callee, id)
            .or_else(|| args.iter().find_map(|e| find_expr_in_expr(e, id))),
        Expr::BinOp { left, right, .. } => {
            find_expr_in_expr(left, id).or_else(|| find_expr_in_expr(right, id))
        }
        Expr::UnOp { operand, .. } | Expr::PostfixTry { inner: operand, .. } => {
            find_expr_in_expr(operand, id)
        }
        Expr::Field { object, .. } => find_expr_in_expr(object, id),
        Expr::Index { object, index, .. } => {
            find_expr_in_expr(object, id).or_else(|| find_expr_in_expr(index, id))
        }
        Expr::StructLit { ty, fields, .. } => find_expr_in_expr(ty, id)
            .or_else(|| fields.iter().find_map(|(_, e)| find_expr_in_expr(e, id))),
        Expr::Block(b) => find_expr_in_block(b, id),
        Expr::List { elements, .. } | Expr::MacroCall { args: elements, .. } => {
            elements.iter().find_map(|e| find_expr_in_expr(e, id))
        }
        _ => None,
    }
}
...
pub fn type_expr_hint(ty: &Type) -> Option<TypeExpr> {
    match ty {
        Type::Int => Some(TypeExpr::Named(Ident::new("number", Span::dummy()))),
        Type::Text => Some(TypeExpr::Named(Ident::new("text", Span::dummy()))),
        _ => None,
    }
}
...
mod tests {
    ...
    fn restores_original_when_recheck_fails() {
        ...
        assert_eq!(
            try_heal_in_place(&Healer, &mut expr, &constraint, || Err::<(), _>(())),
            None
        );
