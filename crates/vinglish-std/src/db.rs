...
fn get_db(_collection: &str) -> Db {
    ...
    let db = DB_CACHE.get_or_init(|| sled::open(".vinglish_db").expect("Failed to open database"));
...
pub fn db_get(collection: &str, key: &str) -> String {
    ...
    if let Ok(tree) = db.open_tree(collection)
        && let Ok(Some(value)) = tree.get(key)
    {
...
pub fn db_batch_put(collection: &str, keys_and_values_json: &str) -> bool {
    ...
    if let Ok(tree) = db.open_tree(collection)
        && let Ok(json) =
            serde_json::from_str::<std::collections::HashMap<String, String>>(keys_and_values_json)
    {
