...
fn fold_instruction<V: Clone + Copy + Display + Eq + Hash>(
    instr: &Instruction<V>,
) -> Option<Instruction<V>> {
    match instr {
...
fn fold_binop(op: BinOp, left: &Literal, right: &Literal) -> Option<Literal> {
    match (left, op, right) {
        ...
        (Literal::Int(a), BinOp::Lt, Literal::Int(b))
        | (Literal::Int(a), BinOp::IsBelow, Literal::Int(b)) => Some(Literal::Bool(a < b)),
        (Literal::Int(a), BinOp::Gt, Literal::Int(b))
        | (Literal::Int(a), BinOp::IsAbove, Literal::Int(b))
        | (Literal::Int(a), BinOp::Exceeds, Literal::Int(b)) => Some(Literal::Bool(a > b)),
