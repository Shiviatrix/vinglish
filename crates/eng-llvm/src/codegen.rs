...
use eng_hir::symbol::{SymbolTable, SsaValueId, FunctionId, TypeId};
use eng_hir::types::Type;
use eng_mir::{MirModule, MirFunction, BasicBlock, Instruction, Operand, Terminator, BlockId};
...
#[derive(Clone, Copy)]
enum ReturnMode {
    /// Function returns void (Englist Unit)
    Void,
...
impl<'ctx> LLVMCodeGen<'ctx> {
    ...
    pub fn compile_module(&mut self, mir: &MirModule<SsaValueId>) -> Result<(), String> {
        ...
        // Verify the module
        self.module.verify().map_err(|e| format!("LLVM verification failed: {}", e.to_string()))
    }
    ...
    fn get_or_create_struct_type(&mut self, type_id: TypeId) -> inkwell::types::StructType<'ctx> {
        ...
        let st = self.type_lowering.lower_struct_type(self.symbol_table, type_id);
    ...
    fn get_func_param_types(&self, func: &MirFunction<SsaValueId>) -> Vec<BasicMetadataTypeEnum<'ctx>> {
        if let Some(fs) = self.symbol_table.get_func(func.id) {
            if let Type::Function(params, _) = &fs.ty {
                return params.iter().map(|p| {
                    match self.type_lowering.lower_type(p, self.symbol_table) {
                        Some(t) => t.into(),
    ...
    fn declare_function(&mut self, func: &MirFunction<SsaValueId>) -> Result<(), String> {
        ...
        // `main` must return i32 for C runtime compatibility, regardless of Englist type
    ...
    fn compile_function(&mut self, func: &MirFunction<SsaValueId>) -> Result<(), String> {
        let llvm_func = *self.func_map.get(&func.id)
        ...
        for (i, &param_id) in func.params.iter().enumerate() {
            let param_val = llvm_func.get_nth_param(i as u32)
    ...
    fn compile_block(&mut self, block: &BasicBlock<SsaValueId>, _func: FunctionValue<'ctx>, mode: ReturnMode) -> Result<(), String> {
        let llvm_bb = *self.block_map.get(&block.id)
    ...
    fn resolve_operand(&self, op: &Operand<SsaValueId>) -> Result<BasicValueEnum<'ctx>, String> {
        match op {
            ...
            Operand::Var(id) => self.ssa_values.get(id)
                .copied()
    ...
    fn lower_literal(&self, lit: &Literal) -> Result<BasicValueEnum<'ctx>, String> {
        match lit {
            ...
            Literal::Bool(b) => Ok(self.context.bool_type().const_int(if *b { 1 } else { 0 }, false).into()),
            Literal::Text(s) => {
    ...
    fn compile_instruction(&mut self, instr: &Instruction<SsaValueId>) -> Result<(), String> {
        match instr {
            ...
            Instruction::HeapAllocate(dest, type_id) => {
                ...
                let size = struct_type.size_of()
                    .ok_or_else(|| "Cannot get struct size".to_string())?;
                let ptr = self.builder.build_call(self.builtins.malloc_fn, &[size.into()], &format!("ssa_{}", dest.0))
                    .map_err(|e| e.to_string())?
                    .try_as_basic_value()
                    .basic()
            ...
            Instruction::StackAllocate(dest, type_id) => {
                let struct_type = self.get_or_create_struct_type(*type_id);
                let alloca = self.builder.build_alloca(struct_type, &format!("ssa_{}", dest.0))
                    .map_err(|e| e.to_string())?;
            ...
            Instruction::LoadField(dest, obj, field_id) => {
                ...
                let struct_type = self.infer_struct_type_for_ptr(obj)
                    .unwrap_or_else(|| self.context.opaque_struct_type("unknown_field_load"));

                let field_ptr = self.builder.build_struct_gep(struct_type, ptr_val, field_idx, &format!("field_{}_ptr", field_idx))
                ...
                let loaded = self.builder.build_load(field_type, field_ptr, &format!("ssa_{}", dest.0))
                    .map_err(|e| e.to_string())?;
                self.ssa_values.insert(*dest, loaded);
            }

            Instruction::StoreField(obj, field_id, val) => {
                let obj_val = self.ssa_values.get(obj).copied()
                ...
                let struct_type = self.infer_struct_type_for_var(*obj)
                    .unwrap_or_else(|| self.context.opaque_struct_type("unknown_field_store"));

                let field_ptr = self.builder.build_struct_gep(struct_type, ptr_val, field_idx, &format!("store_field_{}", field_idx))
            ...
            Instruction::Borrow(dest, src) | Instruction::BorrowMut(dest, src) => {
                ...
                if val.is_pointer_value() {
                    ...
                    let alloca = self.builder.build_alloca(val.get_type(), &format!("borrow_{}", dest.0))
                        .map_err(|e| e.to_string())?;
            ...
            Instruction::Deref(dest, src, type_id) => {
                ...
                let ty = self.symbol_table.get_interned_type(*type_id)
                    .ok_or_else(|| format!("Type {} not found in intern table", type_id.0.0))?;
                
                let llvm_type = self.type_lowering.lower_type(ty, self.symbol_table)
                    .ok_or_else(|| format!("Could not lower type {} for deref", ty))?;
                
                let loaded = self.builder.build_load(llvm_type, ptr, &format!("deref_{}", dest.0))
                    .map_err(|e| e.to_string())?;
                self.ssa_values.insert(*dest, loaded);
            }
            ...
            Instruction::Phi(dest, args) => {
                ...
                let phi = self.builder.build_phi(phi_type, &format!("ssa_{}", dest.0))
                    .map_err(|e| e.to_string())?;
    ...
    fn compile_terminator(&mut self, term: &Terminator<SsaValueId>, mode: ReturnMode) -> Result<(), String> {
        match term {
            ...
            Terminator::Return(Some(op)) => {
                match mode {
                    ...
                    ReturnMode::MainEntrypoint => {
                        let val = self.resolve_operand(op)?;
                        if val.is_int_value() {
                            let i32_val = self.builder.build_int_cast(val.into_int_value(), self.context.i32_type(), "main_ret_cast")
                                .map_err(|e| e.to_string())?;
                            self.builder.build_return(Some(&i32_val)).map_err(|e| e.to_string())?;
                        } else {
                            let zero = self.context.i32_type().const_int(0, false);
                            self.builder.build_return(Some(&zero)).map_err(|e| e.to_string())?;
                        }
            ...
            Terminator::Jump(target) => {
                let bb = self.block_map.get(target)
                    .ok_or_else(|| format!("Jump target {} not found", target))?;
                self.builder.build_unconditional_branch(*bb).map_err(|e| e.to_string())?;
            }
            Terminator::Branch(cond, true_block, false_block) => {
                let cond_val = self.resolve_operand(cond)?;
                let cond_int = cond_val.into_int_value();
                let true_bb = self.block_map.get(true_block)
                    .ok_or_else(|| format!("True branch {} not found", true_block))?;
                let false_bb = self.block_map.get(false_block)
                    .ok_or_else(|| format!("False branch {} not found", false_block))?;
                self.builder.build_conditional_branch(cond_int, *true_bb, *false_bb)
                    .map_err(|e| e.to_string())?;
            }
        }
        Ok(())
    ...
    fn compile_binop(&self, op: BinOp, lhs: BasicValueEnum<'ctx>, rhs: BasicValueEnum<'ctx>, name: &str) -> Result<BasicValueEnum<'ctx>, String> {
        // Determine if we're doing integer or float arithmetic
        if lhs.is_int_value() && rhs.is_int_value() {
            let l = lhs.into_int_value();
            let r = rhs.into_int_value();
            let result: IntValue<'ctx> = match op {
                BinOp::Add => self.builder.build_int_add(l, r, name).map_err(|e| e.to_string())?,
                BinOp::Sub => self.builder.build_int_sub(l, r, name).map_err(|e| e.to_string())?,
                BinOp::Mul => self.builder.build_int_mul(l, r, name).map_err(|e| e.to_string())?,
                BinOp::Div => self.builder.build_int_signed_div(l, r, name).map_err(|e| e.to_string())?,
                BinOp::Mod => self.builder.build_int_signed_rem(l, r, name).map_err(|e| e.to_string())?,
                BinOp::Eq => self.builder.build_int_compare(IntPredicate::EQ, l, r, name).map_err(|e| e.to_string())?,
                BinOp::NotEq => self.builder.build_int_compare(IntPredicate::NE, l, r, name).map_err(|e| e.to_string())?,
                BinOp::Lt => self.builder.build_int_compare(IntPredicate::SLT, l, r, name).map_err(|e| e.to_string())?,
                BinOp::Gt => self.builder.build_int_compare(IntPredicate::SGT, l, r, name).map_err(|e| e.to_string())?,
                BinOp::LtEq => self.builder.build_int_compare(IntPredicate::SLE, l, r, name).map_err(|e| e.to_string())?,
                BinOp::GtEq => self.builder.build_int_compare(IntPredicate::SGE, l, r, name).map_err(|e| e.to_string())?,
                BinOp::And => self.builder.build_and(l, r, name).map_err(|e| e.to_string())?,
                BinOp::Or => self.builder.build_or(l, r, name).map_err(|e| e.to_string())?,
                BinOp::IsBelow => self.builder.build_int_compare(IntPredicate::SLT, l, r, name).map_err(|e| e.to_string())?,
                BinOp::IsAbove => self.builder.build_int_compare(IntPredicate::SGT, l, r, name).map_err(|e| e.to_string())?,
                BinOp::Exceeds => self.builder.build_int_compare(IntPredicate::SGT, l, r, name).map_err(|e| e.to_string())?,
            };
            Ok(result.into())
            ...
            match op {
                BinOp::Add => Ok(self.builder.build_float_add(l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::Sub => Ok(self.builder.build_float_sub(l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::Mul => Ok(self.builder.build_float_mul(l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::Div => Ok(self.builder.build_float_div(l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::Mod => Ok(self.builder.build_float_rem(l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::Eq => Ok(self.builder.build_float_compare(FloatPredicate::OEQ, l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::NotEq => Ok(self.builder.build_float_compare(FloatPredicate::ONE, l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::Lt => Ok(self.builder.build_float_compare(FloatPredicate::OLT, l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::Gt => Ok(self.builder.build_float_compare(FloatPredicate::OGT, l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::LtEq => Ok(self.builder.build_float_compare(FloatPredicate::OLE, l, r, name).map_err(|e| e.to_string())?.into()),
                BinOp::GtEq => Ok(self.builder.build_float_compare(FloatPredicate::OGE, l, r, name).map_err(|e| e.to_string())?.into()),
    ...
    fn compile_unop(&self, op: UnOp, val: BasicValueEnum<'ctx>, name: &str) -> Result<BasicValueEnum<'ctx>, String> {
        match op {
            UnOp::Neg => {
                if val.is_int_value() {
                    Ok(self.builder.build_int_neg(val.into_int_value(), name).map_err(|e| e.to_string())?.into())
                } else if val.is_float_value() {
                    Ok(self.builder.build_float_neg(val.into_float_value(), name).map_err(|e| e.to_string())?.into())
                } else {
            ...
            UnOp::Not => {
                if val.is_int_value() {
                    Ok(self.builder.build_not(val.into_int_value(), name).map_err(|e| e.to_string())?.into())
                } else {
            ...
            UnOp::Borrow(_) => {
                Err("Borrow should be an Instruction, not a UnaryOp".into())
            }
        }
    }

    fn compile_call(&mut self, func_id: FunctionId, args: &[Operand<SsaValueId>], name: &str) -> Result<Option<BasicValueEnum<'ctx>>, String> {
        ...
        let llvm_func = self.func_map.get(&func_id)
        ...
        let call_result = self.builder.build_call(*llvm_func, &arg_values, name)
            .map_err(|e| e.to_string())?;

        Ok(call_result.try_as_basic_value().basic())
    }

    fn compile_print_call(&mut self, args: &[Operand<SsaValueId>]) -> Result<Option<BasicValueEnum<'ctx>>, String> {
        if args.is_empty() {
            return Ok(None);
        }
        ...
        let (fmt, print_args): (&str, Vec<inkwell::values::BasicMetadataValueEnum<'ctx>>) = if val.is_int_value() {
            ("%lld\n", vec![val.into()])
        } else if val.is_float_value() {
            ("%f\n", vec![val.into()])
        ...
        let mut call_args: Vec<inkwell::values::BasicMetadataValueEnum<'ctx>> = vec![fmt_str.as_pointer_value().into()];
        call_args.extend(print_args);

        self.builder.build_call(self.builtins.printf_fn, &call_args, "printf_call")
            .map_err(|e| e.to_string())?;

        Ok(None)
    }

    fn infer_phi_type(&self, args: &[(Operand<SsaValueId>, BlockId)]) -> Result<BasicTypeEnum<'ctx>, String> {
        for (op, _) in args {
            if let Ok(val) = self.resolve_operand(op) {
    ...
    fn infer_struct_type_for_ptr(&self, op: &Operand<SsaValueId>) -> Option<inkwell::types::StructType<'ctx>> {
    ...
    fn infer_struct_type_for_var(&self, _id: SsaValueId) -> Option<inkwell::types::StructType<'ctx>> {
