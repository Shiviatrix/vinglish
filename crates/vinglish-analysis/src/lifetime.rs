    ...
    fn analyze_function(
        ...
        for block in &func.blocks {
            for (idx, instr) in block.instrs.iter().enumerate() {
                let dests = match instr {
                    vinglish_mir::Instruction::HeapAllocate(dest, _)
                    | vinglish_mir::Instruction::StackAllocate(dest, _) => Some(*dest),
        ...
        for block in &func.blocks {
            for (idx, instr) in block.instrs.iter().enumerate() {
                ...
                match instr {
                    vinglish_mir::Instruction::Assign(_, op)
                    | vinglish_mir::Instruction::UnaryOp(_, _, op)
                    | vinglish_mir::Instruction::Borrow(_, op)
                    | vinglish_mir::Instruction::BorrowMut(_, op)
                    | vinglish_mir::Instruction::LoadField(_, op, _) => {
                        if let Operand::Var(v) = op {
                            uses.push(*v);
                        }
                    }
                    vinglish_mir::Instruction::StoreField(obj, _, val) => {
                        uses.push(*obj);
                        if let Operand::Var(v) = val {
                            uses.push(*v);
                        }
                    }
                    vinglish_mir::Instruction::BinaryOp(_, _, left, right) => {
                    ...
                    vinglish_mir::Instruction::Call(_, _, args) => {
                        for arg in args {
                    ...
                    vinglish_mir::Instruction::Phi(_, args) => {
                        for (op, _) in args {
                            if let Operand::Var(v) = op {
                                uses.push(*v);
                            }
            ...
            match &block.terminator {
                vinglish_mir::Terminator::Return(Some(Operand::Var(v))) => {
                ...
                vinglish_mir::Terminator::Branch(Operand::Var(v), _, _) => {
                    if let Some(alias) = alias_graph.get_alias(*v) {
                        let entry = graph.alias_lifetimes.entry(alias).or_insert(Lifetime {
                            created_at: None,
                            last_used_at: None,
