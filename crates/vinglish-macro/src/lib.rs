...
pub fn vinglish_export(_attr: TokenStream, item: TokenStream) -> TokenStream {
            ...
            if let Type::Path(type_path) = &**ty {
                if let Some(segment) = type_path.path.segments.last() {
                    ...
                    match type_name.as_str() {
                        ...
                        _ => panic!(
                            "Unsupported return type in #[vinglish_export]: {}",
                            type_name
                        ),
                    }
                } else {
    ...
    let interfaces_file = PathBuf::from(manifest_dir)
        .parent()
        .unwrap()
        .join(".vinglish_interfaces.tmp");
    if let Ok(mut file) = OpenOptions::new()
        .create(true)
        .append(true)
        .open(&interfaces_file)
    {
