...
impl<'a> LayoutResolver<'a> {
    ...
    pub fn field_offset(&self, record: TypeId, field: FieldId) -> Result<u32, LayoutError> {
        self.layout_type(record)?
            .fields
            .into_iter()
            .find(|layout| layout.id == field)
            .map(|layout| layout.offset)
            .ok_or(LayoutError::UnknownType(record))
    }
        ...
        match ty {
            ...
            Type::Named(name, _) => {
                let symbol_id = self
                    .symbols
                    .lookup(name)
                    .ok_or_else(|| LayoutError::UnresolvedType(name.clone()))?;
                let id = TypeId(symbol_id);
                if id == containing {
                    return Err(LayoutError::RecursiveByValue(id));
                }
